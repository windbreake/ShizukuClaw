...
def verify_imports():
    ...
    print("✓ 验证导入...")
    try:
        spec = importlib.util.spec_from_file_location("web_server", "src/services/web_server.py")
        # spec.loader.exec_module()  # 不实际加载，避免依赖
        print("  ✓ web_server.py 语法正确")
        
        spec = importlib.util.spec_from_file_location("ai_chat_system", "src/agent/ai_chat_system.py")
        # spec.loader.exec_module()  # 不实际加载，避免依赖  
...
def verify_api_endpoints():
    ...
    # 读取 web_server.py
    with open("src/services/web_server.py", "r", encoding="utf-8") as f:
        content = f.read()
...
def verify_optimization():
    ...
    with open("src/agent/ai_chat_system.py", "r", encoding="utf-8") as f:
        content = f.read()
