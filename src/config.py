...
def _load_system_config():
    system_path = os.path.join(PROJECT_ROOT, 'data', 'system_config.json')
...
def _check_plugins_health():
    ...
    lines = []
    config_path = os.path.join(PROJECT_ROOT, 'data', 'config.json')
    plugin_root = os.path.join(PROJECT_ROOT, 'data', 'plungin')
    builtin_path = os.path.join(PROJECT_ROOT, 'src', 'plugin_framework', 'builtin_plugins.py')
            ...
            try:
                with open(req_path, 'r', encoding='utf-8') as f:
                    for raw_line in f:
                        ...
                        if top_level and importlib.util.find_spec(top_level) is None:
