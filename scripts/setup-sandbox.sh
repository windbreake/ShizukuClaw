...
if [ ! -f "$APP_DIR/runtimes/amala-sandbox/package.json" ]; then
    echo -e "${YELLOW}  创建 amala-sandbox 目录...${NC}"
    mkdir -p "$APP_DIR/runtimes/amala-sandbox"
fi

if command -v node &> /dev/null; then
    cd "$APP_DIR/runtimes/amala-sandbox"
...
mkdir -p "$APP_DIR/agent_datas/workspace"
mkdir -p "$APP_DIR/db/data/sandbox_logs"
mkdir -p "$APP_DIR/logs"

# 设置权限
chmod 755 "$APP_DIR/agent_datas/workspace"
chmod 755 "$APP_DIR/db/data"
...
if command -v docker &> /dev/null; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        if [ -f "$APP_DIR/runtimes/amala-sandbox/Dockerfile" ]; then
            echo -e "${YELLOW}  构建 amala-sandbox 镜像...${NC}"
            docker build -t shizuku-js-sandbox:latest "$APP_DIR/runtimes/amala-sandbox/"
...
# 创建初始化完成文件
echo "version: 1.0" > "$BACKEND_DIR/.sandbox-initialized"
...
echo "2. 运行应用: python -m app.main"
echo "3. 或使用 Docker Compose: docker-compose up -d"
echo ""
echo "可选："
echo "- 查看安全策略: cat backend/app/agent/sandbox_security_policy.json"
echo "- 测试 Python 执行: PYTHONPATH=backend python -c 'from app.agent.agent_sandbox import AgentSandbox; sb = AgentSandbox(\"backend/app/agent_datas/workspace\"); print(sb.execute_python_with_details(\"print(2+2)\"))'"
