...
if not exist "%APP_DIR%\runtimes\amala-sandbox\package.json" (
    echo 创建 amala-sandbox 目录...
    if not exist "%APP_DIR%\runtimes" mkdir "%APP_DIR%\runtimes"
    if not exist "%APP_DIR%\runtimes\amala-sandbox" mkdir "%APP_DIR%\runtimes\amala-sandbox"
)

where node >nul 2>&1
if !errorlevel! equ 0 (
    if not exist "%APP_DIR%\runtimes\amala-sandbox\node_modules" (
        echo 安装 Node.js 依赖...
        cd /d "%APP_DIR%\runtimes\amala-sandbox"
...
echo.
echo [4/5] 创建沙箱目录结构...
if not exist "%APP_DIR%\agent_datas\workspace" mkdir "%APP_DIR%\agent_datas\workspace"
if not exist "%APP_DIR%\db\data\sandbox_logs" mkdir "%APP_DIR%\db\data\sandbox_logs"
...
if !errorlevel! equ 0 (
    ...
    if /i "!BUILD_DOCKER!"=="y" (
        ...
        if exist "%APP_DIR%\runtimes\amala-sandbox\Dockerfile" (
...
echo.
echo 可选：
echo - 查看安全策略: type backend\app\agent\sandbox_security_policy.json
echo.
