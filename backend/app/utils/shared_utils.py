...
def should_search(user_input: str) -> bool:
    ...
    # Execution/debug intent guard: these are agent action requests, not search requests.
    execution_keywords = [
        '运行', '启动', '执行', '调试', '修复', '报错', '错误', '异常',
        'run ', ' start', ' execute', ' debug', ' traceback', 'module not found',
        'pytest', 'py_compile', 'pip install', '.py', '.js', '.ts', '.csproj', '.sln'
    ]
    if any(k in normalized for k in execution_keywords):
        return False
