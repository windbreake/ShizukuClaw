...
// 加载聊天记录
// 加载角色列表到数据库记录选择器
async function loadPersonaOptionsForDatabase() {
  try {
    const select = document.getElementById('db-record-persona-select');
    if (!select) return;
    
    const response = await fetch('/api/personas');
    const data = await response.json();
    const personas = data.personas || data;  // 兼容两种格式
    ...
    if (Array.isArray(personas)) {
      personas.forEach(p => {
        ...
        // 优先使用name，如果没有则使用filename
        option.textContent = p.name || p.filename || '未知角色';
...
async function refreshRecords(){
  try {
    // 获取当前选中的角色
    const personaSelect = document.getElementById('db-record-persona-select');
    const personaFilename = personaSelect ? personaSelect.value : '';
    ...
    // 构建请求URL，如果选择了角色则添加persona_filename参数
    let url = '/api/records?limit=50';
    if (personaFilename) {
      url += `&persona_filename=${encodeURIComponent(personaFilename)}`;
    }
    ...
    if (tbody) {
      ...
      if (data.length === 0) {
        ...
        const personaText = personaFilename ? `（${personaFilename}）` : '';
        tr.innerHTML = `<td colspan="3" class="text-center">暂无聊天记录${personaText}</td>`;
        ...
        displayData.forEach((row,i)=>{
          ...
          if (row.length >= 3) {
            ...
            // 处理字典格式
            const userMessage = row.user_input || row.message || '';
            const botResponse = row.ai_response || row.response || row.reply || '';
            const userMessageDisplay = userMessage.length > 100 ? userMessage.substring(0, 100) + '...' : userMessage;
            const botResponseDisplay = botResponse.length > 100 ? botResponse.substring(0, 100) + '...' : botResponse;
            messageDisplay = `<strong>用户:</strong> ${userMessageDisplay}<br><strong>机器人:</strong> ${botResponseDisplay}`;
          } else {
          ...
          const recordId = (typeof row === 'object' && row !== null) ? (row.id || i+1) : row[0];
          tr.innerHTML = `<td>${i+1}</td>
                          <td>${messageDisplay}</td>
                          <td><button class="btn btn-sm btn-danger" onclick="deleteRecord(${recordId})">删除</button></td>`;
          tbody.appendChild(tr);
        });
      }
    }
...
async function deleteRecord(id){
  ...
  try {
    ...
    const body = {id};
    if (personaFilename) {
      body.persona_filename = personaFilename;
    }
...
async function clearAll(){
  const personaSelect = document.getElementById('db-record-persona-select');
  const personaFilename = personaSelect ? personaSelect.value : '';
  const personaText = personaFilename ? `（${personaFilename}）` : '所有角色';
  
  if(!confirm(`确认清空${personaText}的聊天记录？`)) return;
  try {
    const body = {};
    if (personaFilename) {
      body.persona_filename = personaFilename;
    ...
    await refreshRecords();
    toastr.success(`${personaText}的记录已清空`);
  } catch (e) {
...
async function deleteFirstN(){
  ...
  try {
    ...
    await refreshRecords();
    const personaText = personaFilename ? `（${personaFilename}）` : '';
    toastr.success(`前${n}条记录已删除${personaText}`);
  } catch (e) {
