...
try:
    ...
    class BackgroundScheduler:
        ...
        def add_job(self, func, trigger=None, args=None, id=None, replace_existing=True, **kwargs):
            job_id = id
            if not job_id:
                job_id = str(len(self._jobs) + 1)
            if replace_existing or job_id not in self._jobs:
                self._jobs[job_id] = {
                    'func': func,
                    'trigger': trigger,
                    'args': args or [],
        ...
        def remove_job(self, job_id):
            if job_id in self._jobs:
                ...
                return
            raise JobLookupError(str(job_id))
...
def _repo_root_from_here() -> str:
    # agent_task_scheduler.py 位于 app/agent/，向上一层到 app 根目录
...
class AgentTaskScheduler:
    ...
    def __init__(self, storage_dir: str = 'data/tasks'):
        ...
        self.tasks_file = os.path.join(resolved_storage_dir, 'tasks.json')
        self.scheduler = BackgroundScheduler()
        self.scheduler_available = APSCHEDULER_AVAILABLE
