...
class AgentSandbox:
    ...
    @staticmethod
    def _venv_python_path(base_dir):
        if os.name == 'nt':
    ...
    def _bootstrap_python_env(self, base_dir, install_requirements=False):
        """Ensure sandbox-local python runtime is ready and optionally install requirements."""
        steps = []
        py_exec = self._venv_python_path(base_dir)
        ...
        if not os.path.exists(py_exec):
            ...
            steps.append({
                'step': 'python_venv_create',
                'return_code': int(create_proc.returncode),
                'stdout': self._truncate_text(create_proc.stdout),
                'stderr': self._truncate_text(create_proc.stderr),
            })
        ...
        if os.path.exists(py_exec):
            ...
            steps.append({
                'step': 'python_pip_upgrade',
                'return_code': int(pip_upgrade_proc.returncode),
                'stdout': self._truncate_text(pip_upgrade_proc.stdout),
                'stderr': self._truncate_text(pip_upgrade_proc.stderr),
            })

            req_path = os.path.join(base_dir, 'requirements.txt')
            if install_requirements and os.path.exists(req_path):
                ...
                steps.append({
                    'step': 'python_install_requirements',
                    'return_code': int(req_proc.returncode),
                    'stdout': self._truncate_text(req_proc.stdout),
                    'stderr': self._truncate_text(req_proc.stderr),
                })

        if not os.path.exists(py_exec):
            py_exec = sys.executable

        return {'python_exec': py_exec, 'steps': steps}

    @staticmethod
    def _extract_missing_modules(stderr_text):
        text = str(stderr_text or '')
        names = re.findall(r"No module named ['\"]([A-Za-z0-9_\-.]+)['\"]", text)
        dedup = []
        for name in names:
            pkg = str(name or '').strip()
            if pkg and pkg not in dedup:
                dedup.append(pkg)
        return dedup

    def _pip_install_packages(self, base_dir, py_exec, packages):
        if not packages:
            return None
        proc = subprocess.run(
            [py_exec, '-m', 'pip', 'install'] + list(packages),
            cwd=base_dir,
            capture_output=True,
            text=True,
            timeout=300,
        ...
        return {
            'step': 'python_install_missing_packages',
            'return_code': int(proc.returncode),
            'stdout': self._truncate_text(proc.stdout),
            'stderr': self._truncate_text(proc.stderr),
    ...
    def _scan_project_runtime(self, target_dir):
        """Lightweight project scan stage for debug planning.

        This keeps the Astr-like flow of "scan first, then act" while still using
        the project's existing sandbox and diagnostics pipeline.
        """
        safe_root = os.path.abspath(str(target_dir or self.workspace_dir))
        markers = {
            "sln": [],
            "csproj": [],
            "py": [],
        ...
        ignore_dirs = {
            'bin', 'obj', '.git', '.venv', 'venv', '__pycache__', 'node_modules',
        }
        for root, dirs, files in os.walk(safe_root):
            ...
            for name in files:
                ...
                if lower_name.endswith('.sln'):
                    markers['sln'].append(full_path)
                elif lower_name.endswith('.csproj'):
                    markers['csproj'].append(full_path)
                elif lower_name.endswith('.py'):
                    markers['py'].append(full_path)
                elif lower_name in {'package.json', 'pnpm-lock.yaml', 'package-lock.json', 'yarn.lock'}:
                    markers['node'].append(full_path)
                elif lower_name in {'requirements.txt', 'pyproject.toml', 'setup.py'}:
                    markers['python_meta'].append(full_path)

            # Keep scan cheap and predictable for large repositories.
            if len(markers['py']) >= 200 and (markers['sln'] or markers['csproj']):
                break
        ...
        if markers['sln']:
            ...
            plan.extend(['dotnet_restore', 'dotnet_build'])
        elif markers['csproj']:
            project_type = 'dotnet'
            entry = markers['csproj'][0]
            entry_dir = os.path.dirname(entry)
            plan.extend(['dotnet_restore', 'dotnet_build'])
        else:
            if markers['py']:
                entry = markers['py'][0]
                entry_dir = os.path.dirname(entry)
            if markers['python_meta']:
                plan.append('bootstrap_python_env')
            plan.extend(['py_compile'] + (['pytest'] if True else []))

        if project_type == 'python' and markers['node'] and not markers['py']:
            # Node-like repositories still benefit from a scan report, but we keep
            # execution paths conservative unless the caller explicitly asks for it.
            plan = ['scan', 'python_diagnostics']
    ...
    def run_project_debug(self, target='.', run_tests=True, start_app=False, max_output_chars=12000, external_approval_id=''):
        ...
        try:
            ...
            if not os.path.exists(safe_target):
                return json.dumps({
                    ...
                    'steps': [{
                        ...
                        'return_code': 1,
                        'stdout': '',
                        'stderr': f"Target path does not exist: {safe_target}",
                    }],
                    'summary': 'Project diagnostics failed: target path not found.',
                }, ensure_ascii=False, indent=2)
            if not os.path.isdir(safe_target):
                safe_target = os.path.dirname(safe_target)

            scan = self._scan_project_runtime(safe_target)
            outputs = [{
                'step': 'scan',
                'return_code': 0,
                'stdout': json.dumps(scan, ensure_ascii=False, indent=2),
                'stderr': '',
            }]

            if scan['project_type'] == 'dotnet' and scan.get('entry'):
                entry = scan['entry']
                entry_name = os.path.basename(entry)
                entry_dir = scan['entry_dir']

                restore_proc = subprocess.run(
                    ['dotnet', 'restore', entry_name],
                    cwd=entry_dir,
                    capture_output=True,
                    text=True,
                    encoding='utf-8',
                ...
                outputs.append({
                    'step': 'dotnet_restore',
                    'return_code': restore_proc.returncode,
                    'stdout': restore_proc.stdout,
            ...
            # Python or mixed project diagnostics.

            bootstrap = self._bootstrap_python_env(safe_target, install_requirements=True)
            py_exec = str(bootstrap.get('python_exec') or sys.executable)
            for step in bootstrap.get('steps', []):
                outputs.append(step)

            compile_cmd = [py_exec, '-m', 'py_compile']
            py_files = []
            ...
            if run_tests:
                try:
                    ...
                    outputs.append({
                        'step': 'pytest_check',
                        'return_code': pytest_check.returncode,
                        'stdout': pytest_check.stdout,
                        'stderr': pytest_check.stderr,
                    })

                    if int(pytest_check.returncode) != 0:
                        pytest_install = subprocess.run(
                            [py_exec, '-m', 'pip', 'install', 'pytest'],
                            cwd=safe_target,
                            capture_output=True,
                            text=True,
                            encoding='utf-8',
                        ...
                        outputs.append({
                            'step': 'pytest_install',
                            'return_code': pytest_install.returncode,
                            'stdout': pytest_install.stdout,
                            'stderr': pytest_install.stderr,
                        })

                    result_test = subprocess.run(
                        [py_exec, '-m', 'pytest', '-q', '--ignore=bin', '--ignore=obj', '--ignore=.venv', '--ignore=venv'],
                        cwd=safe_target,
                        capture_output=True,
                        text=True,
                        encoding='utf-8',
    ...
    def github_mcp_action_test(self, owner='modelcontextprotocol', repo='servers'):
        ...
        try:
            from app.frameworks.mcp_manager import get_mcp_manager
    ...
    def execute_python_with_details(self, code_str, filename="temp_script.py"):
        ...
        try:
            ...
            if file_ext == '.js':
                ...
                docker_ok, docker_reason = self._is_docker_runtime_available()
                if prefer_docker and docker_ok:
                    return self._execute_python_in_docker(filename)
                return self._execute_python_local(
                    filename,
                    docker_preferred=prefer_docker,
                    docker_unavailable_reason=docker_reason if prefer_docker else "",
                )
            else:
    ...
    def _is_docker_runtime_available(self):
        ...
        try:
            ...
            if int(ping.returncode) != 0:
                reason = (ping.stderr or ping.stdout or '').strip()
                return False, self._truncate_text(reason or 'Docker daemon unavailable', 240)
            return True, ''
        except subprocess.TimeoutExpired:
            return False, 'Docker daemon ping timed out'
        except Exception as e:
            return False, self._truncate_text(f'Docker check failed: {str(e)}', 240)

    def _execute_python_local(self, filename, docker_preferred=False, docker_unavailable_reason=""):
        start = time.time()
        warning = ""
        if docker_preferred:
            reason = docker_unavailable_reason.strip() if docker_unavailable_reason else "Docker unavailable"
            warning = f"{reason}; fallback to local Python runtime."

        bootstrap = self._bootstrap_python_env(self.workspace_dir, install_requirements=False)
        py_exec = str(bootstrap.get('python_exec') or sys.executable)
        ...
        try:
            ...
            if int(result.returncode) != 0:
                missing_packages = self._extract_missing_modules(result.stderr)
                if missing_packages:
                    install_step = self._pip_install_packages(self.workspace_dir, py_exec, missing_packages)
                    if install_step and int(install_step.get('return_code', 1)) == 0:
                        warning = (warning + ' Auto-installed missing packages: ' + ', '.join(missing_packages)).strip()
                        result = subprocess.run(
                            [py_exec, filename],
                            cwd=self.workspace_dir,
                            capture_output=True,
                        ...
                        warning = (warning + ' Failed to auto-install missing packages: ' + ', '.join(missing_packages)).strip()
    ...
    def execute_javascript_with_amala(self, code_str):
        ...
        try:
            ...
            return self._execute_javascript_in_vm2(
                filename,
                docker_preferred=prefer_docker,
                docker_unavailable_reason=docker_reason if prefer_docker else "",
            )
    ...
    def _execute_javascript_in_vm2(self, filename, docker_preferred=False, docker_unavailable_reason=""):
        ...
        if docker_preferred:
            reason = docker_unavailable_reason.strip() if docker_unavailable_reason else "Docker unavailable"
            warning = f"{reason}; fallback to local Node.js runtime."
        ...
        runner_path = os.path.join(
            self.project_root, 'runtimes', 'amala-sandbox', 'runner.js'
        )
        ...
        if not os.path.exists(runner_path):
            return self._format_exec_result(
                ...
                return_code=-1,
                stdout="",
                stderr=f"vm2 sandbox runner not found at {runner_path}. Run: npm install in backend/app/runtimes/amala-sandbox/",
                timed_out=False,
                duration_ms=int((time.time() - start) * 1000),
