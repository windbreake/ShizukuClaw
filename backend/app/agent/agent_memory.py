...
PERSONA_CONFIG_ROOT = os.path.join(DATA_DIR, 'personas')
...
class AgentMemory:
    ...
    def set_persona_context(self, persona_filename: str = None, bootstrap_from_legacy: bool = False):
        ...
        # Persona scope switched; invalidate stale cache for this key.
        self._context_packet_cache.pop(self.current_persona_key, None)
        return paths
    ...
    def _context_packet_signature(self):
        return (
            self.current_persona_key,
            self._file_mtime(self.memory_rules_path),
            self._file_mtime(self.context_compression_path),
        )
    ...
    def build_context_packet(self, persona_filename: str = None):
        ...
        now = time.time()
        signature = self._context_packet_signature()
        cache_item = self._context_packet_cache.get(self.current_persona_key)
        if cache_item:
            if (
                cache_item.get('signature') == signature
                and (now - float(cache_item.get('ts', 0.0) or 0.0)) <= self._context_packet_cache_ttl_seconds
            ):
                cached_text = str(cache_item.get('text') or '').strip()
                if cached_text:
                    return str(cache_item.get('text'))
        ...
        self._context_packet_cache[self.current_persona_key] = {
            'signature': self._context_packet_signature(),
            'ts': now,
            'text': text,
        }
    ...
    def append_short_term(self, role, content, persona_filename: str = None):
        ...
        # Lightweight mode: keep a bounded rolling window to avoid blocking chat path.
        max_items = max(SHORT_TERM_WINDOW * 10, 200)
        if len(st) > max_items:
            st = st[-max_items:]
    ...
    def summarize_memory(self, messages, persona_filename: str = None):
        ...
        try:
            ...
            self.save_short_term(remaining, persona_filename=persona_filename)
            ...
            # 如果失败，仅保存全部消息防止丢失
            self.save_short_term(messages, persona_filename=persona_filename)
