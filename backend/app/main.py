...
def check_ports():
    """检查常用端口状态"""
    try:
        from app.core.config import CONFIG, SYSTEM_CONFIG_DATA
        ports_to_check = [
...
def full_diagnosis():
    ...
    print(Fore.CYAN + "=" * 50)
    try:
        from app.core.config import check_service_status
...
def run_mode(mode):
    ...
    try:
        if mode == 0:
            from app.services.adapter_service import run_adapter_service
            run_adapter_service()
        elif mode == 1:
            from app.services.web_server import run_terminal_chat
            run_terminal_chat()
        elif mode == 2:
            try:
                from app.core.launcher_boot import ensure_office_dependencies, load_launcher_settings
            ...
            from app.services.web_server import run_web_server
