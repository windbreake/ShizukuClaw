...
class PluginManager:
    ...
    def _load_external_plugins(self) -> None:
        ...
        # Directories to skip (special purpose directories)
        skip_dirs = {'astrbot_plugins', '__pycache__', '.git', 'node_modules'}
    ...
    def _load_external_plugin_project(self, project_dir: str) -> None:
        ...
        if os.path.exists(manifest_path):
            try:
                ...
                print(f"[DEBUG] Loading plugin {project_dir}: module_name={fallback_name}, entry={entry_file}")
            except Exception as exc:
                self.logger.warning("Invalid plugin manifest %s: %s", manifest_path, exc)

        # Auto-check and install plugin dependencies before loading
        from app.core.dependency_checker import check_plugin_dependencies
        check_plugin_dependencies(project_dir)
        ...
        # Use module_name from manifest if available, otherwise use fallback
        ...
        # Add module to sys.modules before executing to make it accessible
        sys.modules[module_name] = module
        print(f"[DEBUG] Added module {module_name} to sys.modules")
        spec.loader.exec_module(module)
        plugin_name = self._register_from_module(module, fallback_name)
        if plugin_name:
            self._plugin_projects[plugin_name] = project_dir
            print(f"[DEBUG] Plugin {plugin_name} loaded successfully, module in sys.modules: {module_name in sys.modules}")
